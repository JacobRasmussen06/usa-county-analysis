...
methodology_ui <- function(id){
  ns <- NS(id)
  repo_url <- "https://github.com/JacobRasmussen06/usa-county-analysis"
  tagList(
    h1("Methodology"),
    p("This page of the dashboard serves as a more techincal explanation of the ",
      ...
      "report, linked in the report's ",
      tags$a(
        href = repo_url, target = "_blank", rel = "noopener noreferrer",
        class = "variable-explorer-link", "GitHub repository"),
      "."),
    h2("Data Collection"),
    div(
      class = "card",
    ...
    div(
      ...
      p("Several different archetypes of U.S. counties were uncovered by clustering methods. ",
        ...
        "went and condensed them into 13 reappearing clusters that appeared in at least one of the clustering methods. ",
        "Further information on each archetype can be found in the report."),
      div(
        class = "archetype-pill-list",
        lapply(
          c(
            "Midwest Suburbia", "Great Plains Rural Areas", "Frigid Retirement Communities",
            "Underserved Southern America", "Rural Appalachia", "Extraction / Border Counties",
            "The Rural Recreational West", "Mid Size Metro Areas", "Rugged Coastal Suburbs",
            "Established Industrial Urban Cores", "Global Super Cities", "Affluent Suburbia",
            "Sunny Retirement Communities"
          ),
          function(name) span(class = "archetype-pill", name)
        )
      )
    ),
    h2("Dashboard"),
    div(
      ...
      p("The dashboard you're reading this on was an idea conceived at the beginning of the project. It seemed like ",
        ...
        "scripts for the dashboard can be found in the ",
        tags$a(
          href = repo_url, target = "_blank", rel = "noopener noreferrer",
          class = "variable-explorer-link", "GitHub repository"
        ),
    ...
    div(
      class = "card",
      p("The Interactive Map was also an idea conceived at the beginning of the project. An interactive map felt like the ultimate headline of the project, ",
        "a way to allow users to explore all of the data used in this project. The map was created as a module in the dashboard to save space and not create ",
        "two Shiny products, but it ultimately could function as a standalone project finisher itself.")
    )
  )
}
